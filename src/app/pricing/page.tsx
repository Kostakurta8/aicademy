...
function PricingContent() {
  ...
  const handleUpgrade = async (priceId: string, planId: string) => {
    if (!priceId) {
      alert('Stripe is not configured yet. Set NEXT_PUBLIC_STRIPE_PRO_MONTHLY_PRICE_ID and NEXT_PUBLIC_STRIPE_PRO_YEARLY_PRICE_ID in your environment.')
      return
    }
    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      {isCanceled && (
        <div className="mb-8 p-4 rounded-2xl bg-orange/10 border border-orange/20 text-center">
          <p className="text-sm text-text-secondary">Payment was canceled. No charges were made.</p>
        </div>
      )}
          ...
          >
            ...
            <div className="text-center mb-6">
              <div className={`w-12 h-12 mx-auto mb-3 rounded-xl flex items-center justify-center ${
                p.id === 'free' ? 'bg-surface-raised' : 'bg-gradient-to-br from-purple to-blue'
              }`}>
                {p.id === 'free' ? (
            ...
            {p.id === 'free' ? (
              <Button variant="secondary" className="w-full" disabled={plan === 'free'}>
                {plan === 'free' ? 'Current Plan' : 'Downgrade'}
              </Button>
            ) : (
              <Button
                className="w-full"
                onClick={() => handleUpgrade((p as { priceId: string }).priceId, p.id)}
                loading={loading === p.id}
                disabled={plan === 'pro'}
                icon={plan === 'pro' ? <Check size={16} /> : <Crown size={16} />}
              >

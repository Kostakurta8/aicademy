...
import { playCorrect, playIncorrect, playXPDing } from "@/lib/sounds";
...
function ReviewTab() {
  ...
  const dueCards = useMemo(() => {
    return cards
      .filter(
        (c) =>
          c.nextReview <= now && (!selectedDeck || c.deck === selectedDeck),
      )
        ...
        <div key={currentCard.id} className="animate-fade-in">
          ...
          >
            <Card
              padding="lg"
              glow={flipped}
              className="min-h-[280px] flex items-center justify-center border-2 transition-colors duration-300"
            >
              <div className="text-center w-full">
                ...
                {!flipped ? (
                  <div key="front" className="animate-fade-in">
                    <Brain
                      size={40}
                      className="text-accent mx-auto mb-6 opacity-30"
                    />
                    <p className="text-2xl font-semibold text-text-primary px-4">
                      {currentCard.front}
                    </p>
                    <p className="text-sm text-text-muted mt-8 opacity-50">
                  ...
                  <div key="back" className="animate-fade-in">
                    ...
                    <p className="text-xl text-text-secondary leading-relaxed px-4">
                      {currentCard.back}
                    </p>
                  </div>
                )}
            ...
            <div className="grid grid-cols-4 gap-3 animate-fade-in">
              ...
              >
                <div className="flex flex-col items-center">
                  ...
                  <span className="text-[10px] opacity-70 font-normal">
                    {getIntervalLabel(currentCard, 1)}
                  </span>
                </div>
              </Button>
              <Button
              ...
              >
                <div className="flex flex-col items-center">
                  <span>Hard</span>
                  <span className="text-[10px] opacity-70 font-normal">
                    {getIntervalLabel(currentCard, 2)}
                  </span>
                </div>
              </Button>
              <Button
              ...
              >
                <div className="flex flex-col items-center">
                  <span>Good</span>
                  <span className="text-[10px] opacity-70 font-normal">
                    {getIntervalLabel(currentCard, 3)}
                  </span>
                </div>
              </Button>
              <Button
                variant="primary"
                size="lg"
                className="flex-1 !bg-green hover:!bg-green/90 !border-green"
                onClick={() => handleAnswer(5)}
              >
                <div className="flex flex-col items-center text-white">
                  <span>Easy</span>
                  <span className="text-[10px] opacity-70 font-normal">
                    {getIntervalLabel(currentCard, 5)}
                  </span>
                </div>
              </Button>
            </div>
...
function CardsTab() {
  ...
  const canCreateDeck = useSubscriptionStore((s) =>
    s.canCreateFlashcardDeck(decks.length),
  );
  ...
  const filtered = filterDeck
    ? cards.filter((c) => c.deck === filterDeck)
    : cards;
              ...
              <div className="flex gap-2 items-end">
                <div className="flex-1">
                  ...
                  <select
                    ...
                    value={form.deck}
                    onChange={(e) =>
                      setForm((f) => ({ ...f, deck: e.target.value }))
                    }
                    className="w-full bg-surface-alt border border-border-subtle rounded-lg px-3 py-2 text-sm text-text-primary focus:outline-none focus:border-accent"
                  >
                    {decks.map((d) => (
                ...
                {!showDeckInput ? (
                  canCreateDeck ? (
                    ...
                    <Link
                      href="/pricing"
                      className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium text-purple bg-purple/10 hover:bg-purple/20 transition-colors"
                    >
                  ...
                  <div className="flex gap-1">
                    ...
                    <Button
                      size="sm"
                      variant="ghost"
                      onClick={() => setShowDeckInput(false)}
                    >
                      <X size={14} />
                    </Button>
                  </div>
          ...
          return (
            ...
            >
              <Card padding="sm" className="flex items-center gap-3">
                <div
                  className={`w-2 h-2 rounded-full flex-shrink-0 ${isDue ? "bg-orange" : "bg-green"}`}
                />
...
function StatsTab() {
  ...
  const learning = cards.filter(
    (c) => c.repetition > 0 && c.repetition < 5,
  ).length;
  const newCards = cards.filter(
    (c) => c.repetition === 0 && c.lastReviewed === null,
  ).length;

  // Average ease factor
  const avgEase =
    cards.length > 0
      ? cards.reduce((a, c) => a + c.easeFactor, 0) / cards.length
      : 2.5;
  ...
  const last7Days = Array.from({ length: 7 }, (_, i) => {
    ...
    return {
      ...
      count: reviewHistory.filter(
        (r) => new Date(r.date).toDateString() === dayStr,
      ).length,
    };
  });
      ...
      <Card padding="md">
        ...
        <div className="space-y-2">
          {[
            ...
            {
              label: "Learning (1-2 reps)",
              count: cards.filter((c) => c.repetition >= 1 && c.repetition <= 2)
                .length,
              color: "bg-orange",
            },
            {
              label: "Familiar (3-4 reps)",
              count: cards.filter((c) => c.repetition >= 3 && c.repetition <= 4)
                .length,
              color: "bg-gold",
            },
            ...
            <div key={row.label} className="flex items-center gap-3">
              <span className="text-xs text-text-secondary w-36">
                {row.label}
              </span>
              <div className="flex-1 h-3 bg-surface-alt rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${cards.length ? (row.count / cards.length) * 100 : 0}%`,
                  }}
                />
              </div>
      ...
      <div className="p-6 md:p-8 max-w-3xl mx-auto">
        ...
        <div className="mb-8 animate-fade-in">
          <h1 className="text-3xl font-bold text-text-primary mb-2 flex items-center gap-3">
            <Layers className="text-accent" /> 🃏 Flashcards
          </h1>
          <p className="text-sm text-text-secondary">
            Review cards to remember what you learned!
          </p>
        </div>

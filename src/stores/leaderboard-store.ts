...
const BOT_NAMES = [
  'PromptQueen', 'NeuralNinja', 'TokenTitan', 'AIExplorer', 'DataDreamer',
  'ByteWizard', 'ModelMaster', 'ChatSensei', 'DeepLearner', 'SyntaxStar',
  'PixelPilot', 'LogicLlama', 'CodeCrusader', 'TensorTribe', 'EpochElite',
  'GradientGuru', 'LayerLion', 'BotBuilder', 'VectorVibes', 'WeightWolf',
  'BiasBreaker', 'LossLegend', 'BatchBoss', 'KernelKid', 'AttentionAce',
  'TransformerTim', 'EmbedEmma', 'FineTuneFox', 'RAGRanger', 'AgentAlpha',
]
...
function generateBots(seed: number, count: number, xpRange: [number, number]): LeaderboardEntry[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    bots.push({ id: `bot-${name}`, name, xp, level, streak, avatar, isCurrentUser: false })
  }
...
interface LeaderboardStore {
  ...
  getGlobalLeaderboard: (userXP: number, userName: string, userLevel: number, userStreak: number) => LeaderboardEntry[]
  getWeeklyLeaderboard: (userXP: number, userName: string, userLevel: number, userStreak: number) => LeaderboardEntry[]
}

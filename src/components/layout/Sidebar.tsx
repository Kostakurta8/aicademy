...
function selectNextModule(s: {
  moduleProgress: Record<string, { completedLessons?: string[] }>;
}) {
  for (let i = 0; i < MODULE_SLUGS_S.length; i++) {
    const done =
      s.moduleProgress[MODULE_SLUGS_S[i]]?.completedLessons?.length || 0;

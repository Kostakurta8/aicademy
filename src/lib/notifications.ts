...
function getPrefs(): NotificationPrefs {
  if (typeof window === 'undefined') return { enabled: false, streakReminder: true, dailyReward: true, lastPermissionCheck: '' }
  try {
    const stored = localStorage.getItem(NOTIFICATION_STORAGE_KEY)
    if (stored) return JSON.parse(stored)
  } catch { /* ignore */ }
  return { enabled: false, streakReminder: true, dailyReward: true, lastPermissionCheck: '' }
}
...
export async function requestNotificationPermission(): Promise<boolean> {
  ...
  savePrefs({ ...getPrefs(), enabled: granted, lastPermissionCheck: new Date().toISOString() })
  return granted
}
